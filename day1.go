...
func prelimTests() {
  if fuel(12) != 2 { log.Fatal("wrong fuel val") }
  if fuel(14) != 2 { log.Fatal("wrong fuel val") }
  if fuel(1969) != 654 { log.Fatal("wrong fuel val") }
  if fuel(100756) != 33583 { log.Fatal("wrong fuel val") }
...
func part1Fuel(masses []string) int64 {
  ...
  for _, line := range masses {
    ...
    if err != nil { log.Fatal("Failed to convert input mass to int") }
